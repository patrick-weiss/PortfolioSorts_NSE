...
plot_boxplot <- function(data, 
  ...
  if(var_spec %in% c("mean", "alpha_CAPM", "alpha_FF3"))  plot <- plot + 
  ...
  if(substr(var_spec, 1, 1) == "t") plot <- plot + 
    geom_hline(yintercept = qnorm(0.975), linetype = "dashed", color = "blue", size = 1)
...
# Boxplots ----------------------------------------------------------------
# Specifciations
figure_specs <- tibble(spec = c("t", "t_CAPM", "t_FF3",
                                "se", "se_CAPM", "se_FF3"),
                       label = c("$t$-statistic", "$t$-statistic", "$t$-statistic",
