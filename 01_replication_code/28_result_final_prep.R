...
data_nse_results <- dbConnect(SQLite(), 
                              "Data/data_grid_results.sqlite", 
                              extended_types = TRUE)
...
data_result <- dbReadTable(data_nse_results, "data_grid_results")
...
data_result <- data_result |> 
  mutate(mean = mean * 100,
         ...
         across(starts_with("alpha"), ~ .x * 100))
