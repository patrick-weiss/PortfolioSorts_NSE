...
data_premium_results_demeaned <- data_premium_results_demeaned |>
  mutate(drop_earnings = ifelse(drop_earnings == "Yes", "Excluded", drop_earnings), 
         drop_earnings = ifelse(drop_earnings == "No", "Included", drop_earnings), 
         drop_bookequity = ifelse(drop_bookequity == "Yes", "Excluded", drop_bookequity), 
         drop_bookequity = ifelse(drop_bookequity == "No", "Included", drop_bookequity))
...
plot_premiums_figure_grouped <- function(data, variable) {
  ...
  if(variable %in% c("formation_time")) {
    data <- data |>
...
for(the_variable in mapping_nodes$node) {
  ...
  # Save the plot
  tikz(plot_name,
       standAlone = T, width = 6.5, height = 8)
  plot(plot)
  dev.off()
